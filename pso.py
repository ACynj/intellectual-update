...
def pso(func,# 目标函数，即需要优化的函数
        dim=2, # 解的维度
        num_particles=30, # 粒子的数量，粒子越多搜索范围越广，但计算量也会增大
        max_iter=200, # 最大迭代次数
        w=0.5,# 惯性权重，控制粒子先前速度对当前速度的影响程度
        c1=1.5, # 个体学习因子，调节粒子向自身历史最优位置飞行的步长
        c2=1.5): # 社会学习因子，调节粒子向全局历史最优位置飞行的步长

    # 加入种子，保证代码可重复性
    np.random.seed(42)

    # 初始化粒子的位置和速度
    particles_position = np.random.uniform(-5,5,(num_particles,dim)) # 生成-5<=position<5服从均匀分布的随机数，生成的维度为num_particles*dim
    ...
    # 全局最优
    gbest_index = np.argmin(particles_pbest_fitness)
...
# 运行粒子群算法
best_solution,best_fitness = pso(rosenbrock)
